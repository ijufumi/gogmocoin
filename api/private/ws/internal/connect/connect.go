...
const host = "wss://api.coin.z.com/ws/private/v1"
...
func (c *Connection) dial() error {
	...
	token := c.accessToken.Get().Token()
	if c.accessToken.IsExpired() {
		log.Println("token is expired")
		token = c.accessToken.Extend().Token()
	}

	c.state.Store(connectionStateConnecting)
	conn, res, err := websocket.DefaultDialer.Dial(fmt.Sprintf("%s/%s", host, token), nil)
	if err != nil {
		c.state.Store(connectionStateClosed)
...
func (c *Connection) Close() {
	if !c.accessToken.IsExpired() {
		// トークンが有効な場合は、トークンを削除する
		_ = c.accessToken.Delete()
	}
